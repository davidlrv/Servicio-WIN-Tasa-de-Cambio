...
namespace ServivioWinTasaCambio
{
    ...
    {       
        ...
        protected override void OnStart(string[] args)
        {
            ...
            BufferPagos = int.Parse(ConfigurationSettings.AppSettings["BufferPagos"]);
            ...
            while (FechaInicial < DateTime.Now)
            {
                int r = SincronizarPagoDGI(FechaInicial, BufferPagos);
                if (r > 0)
                {
                    ...
                    EventLog.WriteEntry(OrigenLog, "Se Sincronizaron  : " + r + " Pagos de dia: " + FechaInicial, EventLogEntryType.SuccessAudit);
                ...
                else
                {
                    ...
                    if (!EventLog.SourceExists(OrigenLog))
                        EventLog.CreateEventSource(OrigenLog, Aplicacion);
                    //EventLog.WriteEntry(OrigenLog, FechaInicial + "=> Se Inicio con Exito el Servicio Web, Proxima Ejecucion : " + DateTime.Now.AddMilliseconds(Intervalo).ToString("dd/MM/yyyy hh:mm:ss tt"), EventLogEntryType.Information);
                }                
            }
            ...
            while (FechaInicial < DateTime.Now)
            {
                ...
                if (r > 0)
                {
                    if (!EventLog.SourceExists(OrigenLog))
                        EventLog.CreateEventSource(OrigenLog, Aplicacion);
                    //EventLog.WriteEntry(OrigenLog, "Se Inicio con Exito el Servicio Web, Proxima Ejecucion : " + DateTime.Now.AddMilliseconds(Intervalo).ToString("dd/MM/yyyy hh:mm:ss tt"), EventLogEntryType.Information);
                    EventLog.WriteEntry(OrigenLog,"Se Sincronizaron  : " + r + " Pagos del dia "+ FechaInicial, EventLogEntryType.SuccessAudit);
